...
describe('QueryHelper', () => {
  test('should ignore empty fields', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_empty_input')
    );
  ...
  test('should generate simple query', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_simple')
    );
  ...
  test('should handle cc evidence tags', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_cc_evidence')
    );
    expect(queryString).toBe(
  ...
  test('should handle ft evidence tags', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_ft_evidence')
    );
    expect(queryString).toBe(
      '((ft_ca_bind:blah) AND (ftev_ca_bind:blahvidence))'
    );
  });

  test('should handle range', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_simple_range')
    );
  ...
  test('should handle ft range and evidence', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_ft_range_evidence')
    );
    expect(queryString).toBe(
      '((ftlen_sites:[10 TO 100]) AND (ftev_sites:blahvidence))'
    );
  });

  test('should handle date range', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_range_date')
    );
  ...
  test('should handle xrefs', () => {
    const queryString = createQueryString(
      clauses.filter(f => f.id === 'field_xref')
    );
